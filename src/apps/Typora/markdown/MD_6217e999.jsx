...
具体逻辑也很简单，当\`script\`标签添加html后，就去加载对应的chunk,例如加载child.chunk.js

~~~js
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["child"],{'./src/child.js': function(){...code}])
~~~

关键点在\`window["webpackJsonp"].push\`方法，我们可以在main.bundle.js中找到这样一段代码:
...
每个\`entry chunk\`都会有webpack runtime代码，且都有独立的chunk cache 和module cache， 为了确保异步chunk仅加载一次且每个\`entry chunk\`要知道已经加载过的chunk,webpack通过以上方法同步每个\`entry chunk\`的chunk cache

当执行chunk代码时，\`window["webpackJsonp"].push\`指向的是最后一个\`entry chunk\`的\`webpackJsonpCallback\`，通过\`webpackJsonpCallback\`将chunkId，moduleId添加到chunk cache中。

\`if(parentJsonpFunction) parentJsonpFunction(data);\`中\`parentJsonpFunction\` 实际是指向上一个\`entry chunk\`的\`webpackJsonpCallback\`，因此可以一直递归同步cache，直到第一个\`entry chunk\`停止，因为第一个\`entry chunk\`的\`parentJsonpFunction\`实际上是\`window["webpackJsonp"]\`数组内置的push方法。`
