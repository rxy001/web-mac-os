...
给你一个 升序排列 的数组 nums ，请你 原地 删除重复出现的元素，使每个元素 只出现一次 ，返回删除后数组的新长度。元素的 相对顺序 应该保持 一致 ，并使用O(1) 额外空间。
...
不使用额外空间：位运算符 ^ , \`^\` 如果对应两个操作位有且仅有一个1时结果为1，其他都是0。
...
1. 遍历nums1中的所有元素，把它存放到map中，其中key就是nums1中的元素，value就是这个元素在数组nums1中出现的次数。
2. 遍历nums2中的所有元素，查看map中是否包含nums2的元素，如果包含，就把当前值加入到集合list中，然后对应的value要减1。
...
1. 如果数组的元素只要有一个不是9，加1之后直接返回即可。
2. 如果数组中的所有元素都是9，数组长度增加1位。
...
把非0的往前移动，j表示第一个0的位置。当循环结束后，j之后的都是0。
